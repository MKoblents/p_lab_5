    ...
    public CommandResponse execute(CommandRequest commandRequest) {
        ...
        if (!(data instanceof Map)) {
            return new CommandResponse(false,data,
                    "Error: Invalid data format for update command", commandRequest.requestId());
        }
        ...
        if (!args.containsKey("id") || !args.containsKey("marine")) {
            return new CommandResponse(false, args,
                    "Error: Update requires both 'id' and 'marine' arguments", commandRequest.requestId());
        }
        Long id = (Long) args.get("id");
        if (id == null || id <= 0) {
            return new CommandResponse(false, null,
                    "Error: Valid ID required for update", commandRequest.requestId());
        }
        if (!collectionManager.isIdInCollection(id)) {
            return new CommandResponse(false,  null,
                    "Error: No element found with ID " + id, commandRequest.requestId());
        }
